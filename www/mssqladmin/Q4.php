...
for($i = 0; $i < $server_cnt; $i++) {
	...
	$data = $model->q4_query1();
	if($data != false) {
		$server_name = (string)$server->ServerName;
		$fields = Array("ServerName", "STK_CODE", "NOW_PRC", "TM");
		
		for($j = 0; $j < count($data); $j++) {
			$data[$j]['ServerName'] = $server_name;
		}
	...
	$data2 = $model->q4_query3();
	if($data2 != false) {
		$fields2 = array_keys($data2[0]);
		echo '<div id="data-table-3-'.$i.'" class="table table-responsive"> </div>';
		echo '</div>';
	
		$table_id2 = 'data-table-3-'.$i;
	
		echo "<script>";
		echo "$('#$table_id2').datatable( {";
		echo "data:".json_encode($data2).",";
		echo "fields:".json_encode($fields2);
		echo "});";
		echo "</script>";
	}
}
