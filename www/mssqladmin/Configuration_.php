...
if (! is_writable ( 'db.vars.php' )) {
	...
	if ($db_hostname && $username && $db_name) {
		if (false) {
			...
			$_CONEXAO = sqlsrv_connect ( "$db_hostname", $connectionInfo );
			
			if ($_CONEXAO) {
				echo "<select name='db_name'>";
				
				$query = "sp_databases";
				$resultado = sqlsrv_query ( $_CONEXAO, $query, Array () );
				while ( $campo = sqlsrv_fetch_array ( $resultado, SQLSRV_FETCH_ASSOC ) ) {
					$banco = $campo ['DATABASE_NAME'];
